  ...
  ({ opacity, scale, borderWidth, size, top }: {
    ...
    <Animated.View
      style={[styles.wave, {
        ...
        opacity, transform: [{ scale }], borderWidth,
        borderColor: COLORS.primary, top, backfaceVisibility: "hidden",
...
const KeyboardPadding = React.memo(() => {
  ...
  useEffect(() => {
    ...
    const onShow = (e: any) => {
      ...
      Animated.timing(pad, { toValue: Math.max(h - insets.bottom, 0), duration: Platform.OS === "ios" ? e?.duration ?? 220 : 140, easing: Easing.out(Easing.quad), useNativeDriver: false }).start();
    };
    const onHide = (e: any) => {
      Animated.timing(pad, { toValue: 0, duration: Platform.OS === "ios" ? e?.duration ?? 220 : 140, easing: Easing.out(Easing.quad), useNativeDriver: false }).start();
    };
...
export default function Login() {
  ...
  useEffect(() => {
    (async () => {
      try { const last = await AsyncStorage.getItem("login.lastEmail"); if (last) setEmail(last); } catch {}
    })();
  ...
  useEffect(() => {
    let mounted = true;
    AccessibilityInfo.isReduceMotionEnabled().then((e) => { if (mounted) setReduceMotion(!!e); });
  ...
  const waves = useRef(
    Array.from({ length: WAVE_COUNT }, (_, i) => ({
      opacity: new Animated.Value(0.18 - i * 0.04),
  ...
  useEffect(() => {
    const buildAnims = () => waves.map((wave, i) =>
      Animated.loop(Animated.parallel([
        Animated.sequence([
          Animated.timing(wave.opacity, { toValue: 0.08, duration: 2400 + i * 260, easing: Easing.inOut(Easing.quad), useNativeDriver: true }),
          Animated.timing(wave.opacity, { toValue: 0.18 - i * 0.04, duration: 2400 + i * 260, easing: Easing.inOut(Easing.quad), useNativeDriver: true }),
        ]),
        Animated.sequence([
          Animated.timing(wave.scale, { toValue: 1.12 + i * 0.12, duration: 2600 + i * 280, easing: Easing.inOut(Easing.quad), useNativeDriver: true }),
          Animated.timing(wave.scale, { toValue: 1, duration: 2600 + i * 280, easing: Easing.inOut(Easing.quad), useNativeDriver: true }),
        ]),
      ]))
    );
    const start = () => {
    ...
    const sub = AppState.addEventListener("change", (s) => { appStateRef.current = s; if (s === "active" && isFocused) start(); else stop(); });
  ...
  const afterAuthSuccess = useCallback(async () => {
    ...
    try {
      ...
      if (pendingRaw) {
        ...
        const challengeId = typeof pending?.challengeId === "string" ? pending.challengeId : null;
        const inviteId = typeof pending?.inviteId === "string" ? pending.inviteId : null;
        if (challengeId && inviteId) {
          InteractionManager.runAfterInteractions(() => {
            if (!isMountedRef.current) return;
            router.replace({ pathname: "/profile/notifications", params: { challengeId, invite: inviteId } });
          });
          return;
        }
        if (challengeId) {
    ...
    InteractionManager.runAfterInteractions(() => { if (isMountedRef.current) router.replace("/"); });
  }, [reduceMotion, router, setGuest]);
  ...
  const handleLogin = useCallback(async () => {
    ...
    if (isOffline) {
      setErrorMessage(t("networkError") || "Problème réseau. Réessaie.");
      if (!reduceMotion) Haptics.notificationAsync(Haptics.NotificationFeedbackType.Error).catch(() => {});
      triggerShake(); submittingRef.current = false; return;
    }
    if (!e || !p) {
...
const handleGoogleSignIn = useCallback(async () => {
  ...
  }, [afterAuthSuccess, t, triggerShake]);
  ...
  useEffect(() => {
    if (errorMessage) { const id = setTimeout(() => setErrorMessage(""), 5000); return () => clearTimeout(id); }
  }, [errorMessage]);
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: COLORS.background }}>
      ...
      <ScrollView
        ...
        contentContainerStyle={[styles.scrollContent, { minHeight: SCREEN_HEIGHT - insets.top - insets.bottom, paddingBottom: SPACING * 2 }]}
      ...
      >
        ...
        <View style={styles.headerContainer}>
          ...
          <Text style={styles.tagline} accessibilityLabel={t("appTagline")}>{t("appTagline")}</Text>
        </View>
        ...
        <Animated.View style={[styles.card, { opacity: cardOpacity, transform: [{ translateY: cardTranslate }] }]}>
          ...
          {errorMessage ? (
            <Animated.View accessibilityRole="alert" accessibilityLiveRegion="polite" style={[styles.errorBanner, { transform: [{ translateX: shake }] }]}>
          ...
          <View style={styles.inputWrap}>
            ...
            />
            {!!email && (
              <TouchableOpacity onPress={() => setEmail("")} hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }} accessibilityLabel={t("clear") || "Effacer"} style={styles.trailingBtn}>
          ...
          <View style={styles.inputWrap}>
            ...
            <TextInput
              ...
              returnKeyType="done"
              onSubmitEditing={!disabledCTA ? handleLogin : undefined}
            />
            <TouchableOpacity
          ...
          {/* ── GUEST ── */}
          <TouchableOpacity
            style={[styles.guestButton, anySocialLoading && styles.disabledButton]}
            disabled={anySocialLoading}
            onPress={async () => {
              if (submittingRef.current) return;
              submittingRef.current = true;
...
const styles = StyleSheet.create({
  scrollContent: { flexGrow: 1, alignItems: "center", justifyContent: "space-between", paddingVertical: SPACING * 2, paddingHorizontal: SPACING },
  wave: { position: "absolute" },
  headerContainer: { width: "100%", alignItems: "center", marginTop: SCREEN_HEIGHT * 0.08, marginBottom: SPACING },
  brandTitle: { fontSize: normalize(34), color: COLORS.text, textAlign: "center", fontFamily: "Comfortaa_700Bold", maxWidth: "100%" },
  highlight: { color: COLORS.primary, fontSize: normalize(48) },
  tagline: { fontSize: normalize(17), color: "rgba(0,0,0,0.65)", textAlign: "center", marginTop: SPACING / 2, fontFamily: "Comfortaa_400Regular", maxWidth: "90%" },
  ...
  dividerText: { fontFamily: "Comfortaa_400Regular", color: COLORS.textMuted, fontSize: normalize(12) },
  inputWrap: { width: "100%", flexDirection: "row", alignItems: "center", backgroundColor: "#fff", borderRadius: normalize(16), borderWidth: 1, borderColor: COLORS.border, paddingHorizontal: 12, marginBottom: 12 },
  leadingIcon: { marginRight: 8, opacity: 0.9 },
  trailingBtn: { marginLeft: 8, padding: 4 },
  input: { flex: 1, height: normalize(52), color: COLORS.inputText, fontSize: normalize(15), fontFamily: "Comfortaa_400Regular" },
  forgotPassword: { alignSelf: "flex-end", color: COLORS.primary, fontSize: normalize(14), fontFamily: "Comfortaa_700Bold", marginTop: 4, marginBottom: 6 },
  loginButton: { width: "100%", maxWidth: CARD_W, backgroundColor: COLORS.button, paddingVertical: normalize(12), borderRadius: normalize(16), alignItems: "center", marginTop: SPACING / 2, borderWidth: 1.5, borderColor: COLORS.primary, shadowColor: COLORS.primary, shadowOffset: { width: 0, height: normalize(3) }, shadowOpacity: 0.25, shadowRadius: normalize(5), elevation: 5 },
  loginBtnContent: { flexDirection: "row", alignItems: "center", gap: 8 },
  disabledButton: { opacity: 0.5 },
  loginButtonText: { color: COLORS.text, fontSize: normalize(16), fontFamily: "Comfortaa_700Bold" },
  guestButton: { width: "100%", paddingVertical: normalize(12), alignItems: "center", justifyContent: "center", marginTop: normalize(8) },
  guestButtonText: { color: COLORS.textMuted, fontSize: normalize(13), fontFamily: "Comfortaa_400Regular", textDecorationLine: "underline" },
  footerInline: { width: "100%", alignItems: "center", marginTop: SPACING },
  signupText: { color: COLORS.text, fontSize: normalize(14), fontFamily: "Comfortaa_400Regular" },
