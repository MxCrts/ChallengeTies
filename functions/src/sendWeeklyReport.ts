...
function formatWeekLabel(mondayDate: Date, lang: string): string {
...
function generateDiagnostic(
  ...
  if (week0Days === 0) return m.zero;
  if (isRising && isConsistent) return m.rising_consistent;
  if (isRising) return m.rising_general;
  if (isFalling && streakBrokenThisWeek) return m.falling_broken;
  if (isFalling) return m.falling_general;
  if (isStable && isConsistent) return m.stable_consistent;
  if (isStable && isIrregular) return m.stable_irregular;
...
function generateGoal(
  week0Days: number,
  weakestDayIndex: number | null,
  lang: string
): { target: number; weakDay: string | null; message: string } {
  ...
  const GOAL_MSGS: Record<string, (target: number, weakDay: string | null) => string> = {
...
export const sendWeeklyReport = onSchedule(
  ...
  async () => {
    ...
    const cutoff = Timestamp.fromDate(threeWeeksAgo);
    ...
    for (const userDoc of usersSnap.docs) {
      tasks.push((async () => {
        try {
          ...
          const lang: string = (data.language || "fr").split("-")[0]; // normalise "fr-FR" → "fr"
          const fcmToken: string | undefined = data.fcmToken || data.expoPushToken;
          ...
          const challengesByDate: Record<string, Set<string>> = {};

          // Stats par challenge pour le rapport
          const challengeStatsMap: Record<string, {
            title: string;
            category: string;
          ...
          allChallenges.forEach((ch: any) => {
            ...
            // Stats challenge
            const markedThisWeek = dates.filter((raw: any) => {
              let key = "";
            ...
            dates.forEach((raw: any) => {
              ...
              if (/^\d{4}-\d{2}-\d{2}$/.test(key)) {
                dateKeys.push(key);
                if (!challengesByDate[key]) challengesByDate[key] = new Set();
          ...
          if (week0MarkedDays.length === 0 && week1Days === 0 && week2Days === 0) return;

          // markedDaysBits [L,M,M,J,V,S,D]
          const markedDaysBits: boolean[] = Array(7).fill(false);
          ...
          const uniqueChallengesThisWeek = new Set(
            week0MarkedDays.flatMap(k => [...(challengesByDate[k] || [])])
          ).size;
          ...
          let weakestDayIndex: number | null = null;
          let minCount = Infinity;
          dayCount.forEach((c, i) => {
            if (c < minCount) { minCount = c; weakestDayIndex = i; }
          });
          ...
          const score = computeMomentumScore(
            week0MarkedDays.length, week1Days, week2Days, uniqueChallengesThisWeek
          );
          const prevScore = computeMomentumScore(
            week1Days, week2Days, week2Days, 1
          );

          // ── Diagnostic + objectif ─────────────────────────────────────
          const diagnostic = generateDiagnostic(
            score, prevScore, week0MarkedDays.length, week1Days,
            streakBrokenThisWeek, lang
          );
          ...
          const bestStreak = Math.max(
            0,
            ...Object.values(challengeStatsMap).map(c => c.streak || 0)
          );
          ...
          const NOTIF: Record<string, { title: string; body: (score: number, days: number) => string }> = {
