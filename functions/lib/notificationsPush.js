"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.onNotificationCreatedSendPush = void 0;
// functions/src/notificationsPush.ts
const firestore_1 = require("firebase-functions/v2/firestore");
const firestore_2 = require("firebase-admin/firestore");
const expo_server_sdk_1 = __importDefault(require("expo-server-sdk"));
const db = (0, firestore_2.getFirestore)();
const expo = new expo_server_sdk_1.default();
/**
 * Rate limit par type (ms)
 * - referral_new_child: 10 min + DIGEST
 * - referral_milestone_unlocked: 2 min (simple RL)
 */
const RATE_LIMIT_MS = {
    referral_new_child: 10 * 60 * 1000,
    referral_milestone_unlocked: 2 * 60 * 1000,
};
const DIGEST_ENABLED = {
    referral_new_child: true,
};
function getRateLimitMs(type) {
    if (!type)
        return 0;
    return RATE_LIMIT_MS[type] ?? 0;
}
function isDigestEnabled(type) {
    if (!type)
        return false;
    return !!DIGEST_ENABLED[type];
}
// ---------------- i18n minimal (tu peux brancher ton système ensuite) ----------------
function renderNotification(lang, titleKey, bodyKey, params) {
    const L = (lang || "en").slice(0, 2);
    const dict = {
        fr: {
            "referral.notif.newChild.title": "Parrainage activé 🎉",
            "referral.notif.newChild.body": "🔥 {{username}} a rejoint ChallengeTies. +10 trophées. Total activés : {{activatedCount}}.",
            "referral.notif.milestoneUnlocked.title": "Palier débloqué 🏆",
            "referral.notif.milestoneUnlocked.body": "Tu as débloqué un bonus de {{bonus}} trophées. Paliers : {{milestones}}. Activés : {{activatedCount}}.",
            // ✅ Digest
            "referral.notif.newChildDigest.title": "Parrainage : +{{count}} 🎉",
            "referral.notif.newChildDigest.body": "Tu as {{count}} nouveau(x) filleul(s) activé(s) en quelques minutes. Dernier : {{lastUsername}}.",
        },
        en: {
            "referral.notif.newChild.title": "Referral activated 🎉",
            "referral.notif.newChild.body": "🔥 {{username}} joined ChallengeTies. +10 trophies. Activated total: {{activatedCount}}.",
            "referral.notif.milestoneUnlocked.title": "Milestone unlocked 🏆",
            "referral.notif.milestoneUnlocked.body": "You unlocked {{bonus}} bonus trophies. Milestones: {{milestones}}. Activated: {{activatedCount}}.",
            // ✅ Digest
            "referral.notif.newChildDigest.title": "Referrals: +{{count}} 🎉",
            "referral.notif.newChildDigest.body": "You got {{count}} new activated referral(s). Latest: {{lastUsername}}.",
        },
    };
    const pack = dict[L] || dict.en;
    const titleTpl = pack[titleKey] || titleKey;
    const bodyTpl = pack[bodyKey] || bodyKey;
    const safeParams = { ...params };
    if (Array.isArray(safeParams.milestones))
        safeParams.milestones = safeParams.milestones.join(", ");
    const apply = (tpl) => tpl.replace(/\{\{(\w+)\}\}/g, (_, k) => safeParams?.[k] != null ? String(safeParams[k]) : "");
    return { title: apply(titleTpl), body: apply(bodyTpl) };
}
exports.onNotificationCreatedSendPush = (0, firestore_1.onDocumentCreated)({
    region: "europe-west1",
    document: "notifications/{notifId}",
}, async (event) => {
    const notifId = event.params.notifId;
    const notifSnap = event.data;
    const data = notifSnap?.data();
    if (!data)
        return;
    // ✅ Anti double-handling
    if (data.sentAt || data.suppressedAt)
        return;
    const userId = data.userId;
    if (!userId)
        return;
    const type = data.type || "generic";
    const windowMs = getRateLimitMs(type);
    const userRef = db.collection("users").doc(userId);
    const notifRef = db.collection("notifications").doc(notifId);
    // ------------------------------------------------------------
    // ✅ DECISION (transaction) : single / digest / skip
    // ------------------------------------------------------------
    let decision = { mode: "send_single" };
    if (windowMs > 0) {
        const now = Date.now();
        decision = await db.runTransaction(async (tx) => {
            const [uSnap, nSnap] = await Promise.all([tx.get(userRef), tx.get(notifRef)]);
            if (!uSnap.exists)
                return { mode: "skip", reason: "user_not_found" };
            if (!nSnap.exists)
                return { mode: "skip", reason: "notif_deleted" };
            const n = nSnap.data();
            if (n.sentAt || n.suppressedAt)
                return { mode: "skip", reason: "already_handled" };
            const u = uSnap.data();
            if (u?.notificationsEnabled === false) {
                tx.update(notifRef, {
                    suppressedAt: firestore_2.FieldValue.serverTimestamp(),
                    suppressedReason: "notifications_disabled",
                });
                return { mode: "skip", reason: "notifications_disabled" };
            }
            const lastSingleTs = u?.debugPush?.lastPushAtByType?.[type] || undefined;
            const lastSingle = lastSingleTs ? lastSingleTs.toMillis() : 0;
            // ✅ Si on peut envoyer un single (hors fenêtre) -> on réserve
            if (!lastSingle || now - lastSingle >= windowMs) {
                tx.update(userRef, {
                    [`debugPush.lastPushAtByType.${type}`]: firestore_2.FieldValue.serverTimestamp(),
                    "debugPush.lastPushType": type,
                    "debugPush.updatedAt": firestore_2.FieldValue.serverTimestamp(),
                });
                return { mode: "send_single" };
            }
            // ----------------------------------------------------------------
            // ✅ Sinon : rate-limited
            //    - si digest activé : on queue et éventuellement on déclenche un digest
            //    - sinon : on suppress "rate_limited"
            // ----------------------------------------------------------------
            if (!isDigestEnabled(type)) {
                tx.update(notifRef, {
                    suppressedAt: firestore_2.FieldValue.serverTimestamp(),
                    suppressedReason: "rate_limited",
                    suppressedMeta: { type, windowMs, lastMsAgo: now - lastSingle },
                });
                return { mode: "skip", reason: "rate_limited" };
            }
            // ✅ DIGEST QUEUE
            const lastUsername = (typeof n?.params?.username === "string" && n.params.username) || "Someone";
            const digestPathBase = `debugPush.digestByType.${type}`;
            const pending = Number(u?.debugPush?.digestByType?.[type]?.pendingCount || 0);
            const lastDigestSentTs = u?.debugPush?.digestByType?.[type]?.lastDigestSentAt || undefined;
            const lastDigestSent = lastDigestSentTs ? lastDigestSentTs.toMillis() : 0;
            const nextPending = pending + 1;
            // On supprime le push individuel, mais on garde la notif in-app
            tx.update(notifRef, {
                suppressedAt: firestore_2.FieldValue.serverTimestamp(),
                suppressedReason: "digest_queued",
                suppressedMeta: { type, windowMs },
            });
            // On stocke le digest state
            tx.update(userRef, {
                [`${digestPathBase}.pendingCount`]: nextPending,
                [`${digestPathBase}.lastUsername`]: lastUsername,
                [`${digestPathBase}.updatedAt`]: firestore_2.FieldValue.serverTimestamp(),
            });
            // ✅ Si fenêtre digest dépassée, on déclenche un digest maintenant
            if (!lastDigestSent || now - lastDigestSent >= windowMs) {
                tx.update(userRef, {
                    [`${digestPathBase}.lastDigestSentAt`]: firestore_2.FieldValue.serverTimestamp(),
                    [`${digestPathBase}.pendingCount`]: 0, // reset (réservation anti double)
                });
                return {
                    mode: "send_digest",
                    digest: { count: nextPending, lastUsername },
                };
            }
            return { mode: "skip", reason: "digest_queued" };
        });
        if (decision.mode === "skip")
            return;
    }
    // ------------------------------------------------------------
    // ✅ Fetch user + tokens + lang (pour envoyer push)
    // ------------------------------------------------------------
    const userSnap = await userRef.get();
    if (!userSnap.exists)
        return;
    const user = userSnap.data();
    if (user?.notificationsEnabled === false) {
        await notifRef.update({
            suppressedAt: firestore_2.FieldValue.serverTimestamp(),
            suppressedReason: "notifications_disabled",
        });
        return;
    }
    const tokens = Array.isArray(user.expoPushTokens)
        ? user.expoPushTokens
        : typeof user.expoPushToken === "string"
            ? [user.expoPushToken]
            : [];
    const validTokens = tokens.filter((t) => expo_server_sdk_1.default.isExpoPushToken(t));
    if (validTokens.length === 0) {
        await notifRef.update({
            suppressedAt: firestore_2.FieldValue.serverTimestamp(),
            suppressedReason: "no_expo_token",
        });
        return;
    }
    const lang = (typeof user.language === "string" && user.language) ||
        (typeof user.lang === "string" && user.lang) ||
        "en";
    // ------------------------------------------------------------
    // ✅ Build payload : single vs digest
    // ------------------------------------------------------------
    let title = "";
    let body = "";
    let pushType = type;
    let pushData = { type: pushType, notifId };
    if (decision.mode === "send_digest") {
        const { count, lastUsername } = decision.digest;
        const rendered = renderNotification(lang, "referral.notif.newChildDigest.title", "referral.notif.newChildDigest.body", { count, lastUsername });
        title = rendered.title;
        body = rendered.body;
        pushType = "referral_new_child_digest";
        pushData = { type: pushType, count, lastUsername };
    }
    else {
        // send_single (notif normale)
        const titleKey = data.titleKey || "";
        const bodyKey = data.bodyKey || "";
        const params = data.params || {};
        const rendered = renderNotification(lang, titleKey, bodyKey, params);
        title = rendered.title;
        body = rendered.body;
        pushData = { type: pushType, notifId, ...params };
    }
    const messages = validTokens.map((to) => ({
        to,
        title,
        body,
        sound: "default",
        data: pushData,
    }));
    try {
        const chunks = expo.chunkPushNotifications(messages);
        for (const chunk of chunks) {
            // eslint-disable-next-line no-await-in-loop
            await expo.sendPushNotificationsAsync(chunk);
        }
        // ✅ On marque sentAt seulement pour les singles.
        // Les digests sont “système” et ne correspondent pas à la notif doc.
        if (decision.mode === "send_single") {
            await notifRef.update({
                sentAt: firestore_2.FieldValue.serverTimestamp(),
            });
        }
        else {
            // option: log/debug
            await userRef.update({
                "debugPush.lastDigestType": type,
                "debugPush.lastDigestAt": firestore_2.FieldValue.serverTimestamp(),
            });
        }
    }
    catch (e) {
        if (decision.mode === "send_single") {
            await notifRef.update({
                sendError: String(e?.message || e),
                sendErrorAt: firestore_2.FieldValue.serverTimestamp(),
            });
        }
        else {
            await userRef.update({
                "debugPush.lastDigestError": String(e?.message || e),
                "debugPush.lastDigestErrorAt": firestore_2.FieldValue.serverTimestamp(),
            });
        }
    }
});
