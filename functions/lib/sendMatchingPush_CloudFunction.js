...
exports.sendMatchingPush = void 0;
...
exports.sendMatchingPush = (0, https_1.onCall)({ region: "europe-west1" }, async (request) => {
    if (!request.auth) {
        throw new https_1.HttpsError("unauthenticated", "Non authentifié.");
    }
    const { type, inviteeId, inviterUsername, challengeTitle, challengeId, selectedDays, inviteId } = request.data;
    if (!inviteeId || !type) {
    ...
    try {
        ...
        const language = String(userData?.language || "en").split("-")[0].toLowerCase();
        if (!token)
