...
async function googleTranslate(texts, targetLang, apiKey) {
...
exports.translateContent = (0, https_1.onCall)({ region: "europe-west1", secrets: ["GOOGLE_TRANSLATE_API_KEY"] }, async (request) => {
    const { type, docId, targetLang, subDocId } = request.data;
    ...
    if (!API_KEY) {
        throw new https_1.HttpsError("internal", "Translation API key not configured");
    }
    ...
    if (type === "challenge") {
        ...
        if (!data.creatorId) {
            throw new https_1.HttpsError("failed-precondition", "Official challenges use i18n");
        }
        const title = String(data.title || "");
        const description = String(data.description || "");
        if (!title && !description) {
            throw new https_1.HttpsError("failed-precondition", "No content to translate");
        }
    ...
    if (type === "review") {
        if (!subDocId) {
            throw new https_1.HttpsError("invalid-argument", "subDocId required for reviews");
        }
        const translationRef = db
        ...
        if (!text) {
            throw new https_1.HttpsError("failed-precondition", "No text to translate");
        }
    ...
    if (type === "feature") {
        ...
        if (!featureSnap.exists) {
            throw new https_1.HttpsError("not-found", "Feature not found");
        }
    ...
    throw new https_1.HttpsError("invalid-argument", "Unknown type");
});
